...
from spo_service.faiss_request import RequestFaiss
from spo_service.rank_request import RequestRank
    ...
    def process_faiss_ret(self, query, faiss_ret):
        ...
        if "s" in faiss_ret and "p" in faiss_ret:
            ...
            for ind in range(len(passages)):
                ...
                self.tp_sid_map[answer[0] + answer[1]] = [answer[2], distances[ind]]
        return qp
                ...
                try:
                    ...
                    if len(qp) > 0:
                        ...
                        for i, ind in enumerate(res_score_index):
                            sid = self.tp_sid_map[qp[ind][1] + qp[ind][2]][0]
                            rs = self.tp_sid_map[qp[ind][1] + qp[ind][2]][1]
                            res = {'sid':sid, 't':qp[ind][1], 'p':qp[ind][2], 's':res_score[i], 'q': query, 'rs':rs}
                            self.index_result.append(res)
                                #print(tp)
                        # 请求阅读理解
                        #if len(tps) > 0:
...
class GetQueryRes(object):
    ...
    def get_query_res(self, recall_bns_addr, rank_bns_addr, search_ip_addr, mrc_ip_addr, file_id, fin):
        ...
        # 日志处理
        Req_Logger.info("search result: %s" % json.dumps(q_req.index_result, ensure_ascii=False))
